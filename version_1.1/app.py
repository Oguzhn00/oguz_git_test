...
def acquisition_worker(cmd_q, out_q, evt_q, stop_ev):
    ...
    st = {"use_hw": False, "params": SensorParams(), "tab": 1,
          "bg": None, "bg_meta": None, "subtract": False,
    ...
    def handle(cmd):
        ...
        if t == "params":
            ...
            st["bg"] = cmd["bg"]; st["bg_meta"] = cmd["meta"]
        elif t == "clear_bg":
            st["bg"] = None; st["bg_meta"] = None
        elif t == "rec_start":
    ...
    while not stop_ev.is_set():
        ...
        try:
            res = rc.process_frame(frame, p, st["tab"])
        except Exception as e:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        self.thr_combo = QtWidgets.QComboBox(); self.thr_combo.addItems(["Gürültü tabanı", "CFAR"]); self.thr_combo.currentTextChanged.connect(self._apply)
    ...
    def _apply(self, *_):
        ...
        p.threshold_method = "CFAR" if self.thr_combo.currentText() == "CFAR" else "Sabit"
    ...
    def _bg_save(self):
        ...
        if path:
            rc.save_background(path, self.background, self.params)
    ...
    def _bg_load(self):
        ...
        if not path:
            return
        try:
            self.background, self.bg_meta = rc.load_background(path)
            self.cmd_q.put({"type": "set_bg", "bg": self.background, "meta": self.bg_meta})
    ...
    def _reset_defaults(self, *_):
        ...
        self.cmd_q.put({"type": "clear_bg"}); self.background = None
        self._apply()
