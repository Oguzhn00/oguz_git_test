...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        # Parametre profili (preset) — aynı ortamda ayarları tekrar girmemek için
        g, l = group("Parametre Profili (Ayar Dosyası)")
        pb = QtWidgets.QHBoxLayout()
        save_p = QtWidgets.QPushButton("Ayarları Kaydet"); save_p.clicked.connect(self._preset_save)
        load_p = QtWidgets.QPushButton("Ayarları Yükle"); load_p.setObjectName("primary"); load_p.clicked.connect(self._preset_load)
        pb.addWidget(save_p); pb.addWidget(load_p); l.addLayout(pb)
        self.preset_status = hint("Tüm ayarları (.json) kaydet; sonra tek tıkla yükle — "
                                  "aynı ortamda parametreleri tekrar girmene gerek kalmaz. "
                                  "Ölçüm aralığı, TX/kazanç, PRF, zamanlama, işleme ve eşik "
                                  "ayarlarının tamamı dosyaya yazılır.")
        l.addWidget(self.preset_status)
        v.addWidget(g)
        ...
        # CFAR ayarları — FİZİKSEL MESAFE (cm) cinsinden; step_length değişse de
        # korunan/örneklenen bölge aynı kalır. (Yalnızca CFAR yönteminde etkili.)
        self.sl_guard = Slider("CFAR guard", 0, 100, int(self.params.cfar_guard_m * 100), fmt="{} cm")
        self.sl_guard.changed.connect(self._apply); l.addWidget(self.sl_guard)
        self.sl_cwin = Slider("CFAR referans", 5, 200, int(self.params.cfar_window_m * 100), fmt="{} cm")
        self.sl_cwin.changed.connect(self._apply); l.addWidget(self.sl_cwin)
        self.cfar_lb = hint("")
        l.addWidget(self.cfar_lb)
        # NMS: yan lob (ringing) bastırma — tüm eşik yöntemlerinde etkili
        self.sl_nms = Slider("NMS ayrım", 0, 100, int(self.params.nms_min_dist_m * 100), fmt="{} cm")
        self.sl_nms.changed.connect(self._apply); l.addWidget(self.sl_nms)
        self.nms_lb = hint("")
        l.addWidget(self.nms_lb)
    ...
    def _build_raw_tab(self):
        ...
        # --- İMLEÇ OKUMASI: fareyi gezdir, o mesafedeki genlik DEĞERİNİ oku ---
        self.amp_vline = pg.InfiniteLine(angle=90, movable=False,
                                         pen=pg.mkPen(MUTED, style=QtCore.Qt.DashLine))
        self.amp_hline = pg.InfiniteLine(angle=0, movable=False,
                                         pen=pg.mkPen(MUTED, style=QtCore.Qt.DashLine))
        self.amp_plot.addItem(self.amp_vline, ignoreBounds=True)
        self.amp_plot.addItem(self.amp_hline, ignoreBounds=True)
        self.amp_readout = pg.TextItem(color=TEXT, anchor=(0, 1),
                                       fill=pg.mkBrush(0, 0, 0, 160))
        self.amp_plot.addItem(self.amp_readout)
        self._amp_r = None; self._amp_vals = None      # imlecin okuyacağı son veri
        self.amp_plot.scene().sigMouseMoved.connect(self._on_amp_mouse)
    ...
    def _apply(self, *_):
        ...
        p.cfar_guard_m = self.sl_guard.value() / 100.0
        p.cfar_window_m = self.sl_cwin.value() / 100.0
        p.nms_min_dist_m = self.sl_nms.value() / 100.0
    ...
    def _update_derived_labels(self):
        ...
        # CFAR: cm -> hücre dönüşümünü canlı göster (çekirdekle aynı hesap)
        step_m = max(p.step_m(), 1e-6)
        eff_guard, eff_window = rc.cfar_cells(p, p.num_points)
        active = (p.threshold_method == "CFAR")
        col2 = ACCENT if active else MUTED
        note = "" if active else " — yalnızca CFAR yönteminde etkili"
        auto = " (otomatik: darbe genişliği)" if p.cfar_guard_m <= 0 else ""
        self.cfar_lb.setText(
            f"<span style='color:{col2}'>→ guard ±{eff_guard*step_m*100:.1f} cm "
            f"({eff_guard} hücre){auto}, referans {eff_window*step_m*100:.1f} cm "
            f"({eff_window} hücre/yön){note}</span>")
        nms_on = p.nms_min_dist_m > 0
        self.nms_lb.setText(
            f"<span style='color:{ACCENT if nms_on else MUTED}'>→ NMS "
            f"{'açık: ' + f'{p.nms_min_dist_m*100:.0f} cm içindeki daha zayıf tepeler yan lob sayılıp silinir' if nms_on else 'kapalı (0 cm): tüm tepeler gösterilir'}</span>")
    ...
    def _preset_save(self):
        """Tüm ayarları .json profiline yaz (aynı ortamda tekrar kullanmak için)."""
        path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Ayarları kaydet", "ayarlar.json", "Ayar profili (*.json)")
        if not path:
            return
        try:
            extra = {
                "subtract": self.bg_subtract.isChecked(),
                "adaptive": self.bg_adaptive.isChecked(),
                "noise_scale": self.sl_noise.value(),
                "leakage": self.leak_check.isChecked(),
                "targets": [list(t) for t in self.sim_targets],
                "source": self.source_combo.currentText(),
                "conn": self.conn_combo.currentText(),
                "port": self.hw_port.text(),
            }
            rc.save_preset(path, self.params, extra=extra)
            self.preset_status.setText(f"Kaydedildi: {os.path.basename(path)}")
            self.preset_status.setStyleSheet(f"color:{ACCENT};")
        except Exception as e:
            self.preset_status.setText(f"Hata: {e}"); self.preset_status.setStyleSheet(f"color:{DANGER};")

    def _preset_load(self):
        """.json profilini yükle: tüm kontroller doldurulur ve hemen uygulanır."""
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Ayarları yükle", "", "Ayar profili (*.json)")
        if not path:
            return
        try:
            p, extra = rc.load_preset(path)
            self.params = p
            self._params_to_ui(p, extra)      # kontrolleri doldur
            self._apply()                     # worker'a gönder
            self.preset_status.setText(f"Yüklendi: {os.path.basename(path)} — ayarlar uygulandı.")
            self.preset_status.setStyleSheet(f"color:{ACCENT};")
        except Exception as e:
            self.preset_status.setText(f"Hata: {e}"); self.preset_status.setStyleSheet(f"color:{DANGER};")

    def _params_to_ui(self, p, extra=None):
        """SensorParams + extra -> arayüz kontrolleri (sinyaller susturularak)."""
        extra = extra or {}
        widgets = (self.sp_start, self.sp_num, self.step_combo, self.sp_spf, self.sp_rate,
                   self.tx_db, self.prf_combo, self.tx_check, self.amp_combo, self.dopp_combo,
                   self.win_combo, self.thr_combo, self.leak_check, self.bg_subtract,
                   self.bg_adaptive, self.source_combo, self.conn_combo, self.hw_port)
        for w in widgets:
            w.blockSignals(True)
        try:
            self.sp_spf.setMaximum(512)
            self.sp_start.setValue(p.start_point); self.sp_num.setValue(p.num_points)
            self.step_combo.setCurrentText(str(p.step_length))
            self.sp_spf.setValue(p.sweeps_per_frame); self.sp_rate.setValue(p.sweep_rate)
            self.tx_db.setValue(rc.profile_tx_db(p.profile))
            self.prf_combo.setCurrentText(p.prf_key); self.tx_check.setChecked(p.enable_tx)
            self.amp_combo.setCurrentText(p.amplitude_method)
            self.dopp_combo.setCurrentText(p.doppler_method)
            self.win_combo.setCurrentText({"none": "Pencere yok", "hamming": "Hamming",
                                           "hann": "Hann"}.get(p.window_type, "Pencere yok"))
            self.thr_combo.setCurrentText({"CFAR": "CFAR", "Kayıtlı": "Kayıtlı eşik"}.get(
                p.threshold_method, "Gürültü tabanı"))
            if "subtract" in extra:
                self.bg_subtract.setChecked(bool(extra["subtract"]))
            if "adaptive" in extra:
                self.bg_adaptive.setChecked(bool(extra["adaptive"]))
            if "leakage" in extra:
                self.leak_check.setChecked(bool(extra["leakage"]))
            if extra.get("source"):
                self.source_combo.setCurrentText(extra["source"])
            if extra.get("conn"):
                self.conn_combo.setCurrentText(extra["conn"])
            if extra.get("port") is not None:
                self.hw_port.setText(extra["port"])
        finally:
            for w in widgets:
                w.blockSignals(False)
        # kaydırıcılar (kendi blockSignals'ını set_value içinde yapar)
        ...
        self.sl_floor.set_value(int(p.detection_floor))
        self.sl_guard.set_value(int(p.cfar_guard_m * 100)); self.sl_cwin.set_value(int(p.cfar_window_m * 100))
        self.sl_nms.set_value(int(p.nms_min_dist_m * 100))
        if "noise_scale" in extra:
            self.sl_noise.set_value(int(extra["noise_scale"]))
        if extra.get("targets"):
            self.sim_targets = [tuple(t) for t in extra["targets"]]
            self._refresh_table()
        # kaynak görünürlüğünü güncelle
        self._on_source(self.source_combo.currentText())
    ...
    def _on_amp_mouse(self, pos):
        # Fare genlik grafiğinde gezerken, en yakın mesafe noktasının genlik
        # DEĞERİNİ sayı olarak göster (imleç + etiket).
        if self._amp_r is None or not len(self._amp_r):
            return
        pitem = self.amp_plot.getPlotItem()
        if not pitem.sceneBoundingRect().contains(pos):
            self.amp_readout.setText("")
            return
        mp = pitem.vb.mapSceneToView(pos)
        idx = int(np.argmin(np.abs(self._amp_r - mp.x())))
        rx = float(self._amp_r[idx]); av = float(self._amp_vals[idx])
        self.amp_vline.setPos(rx); self.amp_hline.setPos(av)
        self.amp_readout.setText(f"{rx*100:.1f} cm\ngenlik = {av:.1f}")
        self.amp_readout.setPos(rx, av)
    ...
    def _draw_raw(self, res):
        ...
        self._amp_r = r; self._amp_vals = amp        # imleç bu son veriyi okur
        ...
        rmag = np.abs(raw)   # ham IQ matrisinin genlik (büyüklük) değerleri
        self.iq_info.setText(
            f"Tepe genlik: {amp[pk]:.1f} @ {r[pk]*100:.1f} cm  •  "
            f"genlik min / ort / maks: {amp.min():.1f} / {amp.mean():.1f} / {amp.max():.1f}  •  "
            f"ham |IQ| maks: {rmag.max():.1f}  •  SNR ≈ {res['snr']:.1f} dB  •  "
    ...
    def _reset_defaults(self, *_):
        ...
        p = SensorParams()
        self.params = p
        # tüm kontrolleri varsayılan parametrelerden doldur (CFAR dahil, tek yer)
        self._params_to_ui(p, extra={"subtract": False, "adaptive": False,
                                     "leakage": True, "noise_scale": 100})
