...
class SensorParams:
    ...
    cfar_guard_m: float = 0.0            # CFAR koruma bölgesi [m] (tek yön). 0 -> OTOMATİK:
                                         # darbe genişliğinden (profil) hesaplanır.
                                         # Nokta sayısı DEĞİL: step_length ne olursa olsun
                                         # aynı fiziksel mesafeye karşılık gelir.
    cfar_window_m: float = 0.40          # CFAR referans penceresi [m] (tek yön). Yerel
                                         # gürültünün kestirileceği fiziksel bölge.
    nms_min_dist_m: float = 0.20         # NMS: bir tepenin bu mesafedeki daha zayıf
                                         # tepeleri yan lob sayılıp bastırılır. 0 -> kapalı.
...
# ---------------------------------------------------------------------------
# 6b) PARAMETRE PROFİLİ (PRESET) KAYDET / YÜKLE
#     Amaç: aynı ortamda tekrar tekrar aynı ayarları elle girmemek.
#     Tüm SensorParams alanları + sahne/opsiyonlar JSON olarak yazılır (insan
#     okunur; istenirse metin düzenleyiciyle de değiştirilebilir).
# ---------------------------------------------------------------------------
def save_preset(path: str, p: SensorParams, extra: dict = None) -> None:
    """
    Parametreleri .json profiline yaz. extra: arayüz tarafı ek ayarlar
    (arka plan çıkarma açık mı, adaptif mi, sim sahnesi vb.).
    """
    import json
    from dataclasses import asdict
    data = {"a121_preset": 1, "params": asdict(p), "extra": (extra or {})}
...
def load_preset(path: str):
    """
    .json profilini oku. Dönüş: (SensorParams, extra[dict]).
    Bilinmeyen/eksik alanlar yok sayılır; varsayılanlar korunur (ileri uyumluluk).
    """
    import json
    ...
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    p = SensorParams()
    valid = {fl.name for fl in fields(SensorParams)}
    for k, v in (data.get("params") or {}).items():
        if k in valid:
            setattr(p, k, v)
    return p, (data.get("extra") or {})
...
def process_frame(frame: np.ndarray, p: SensorParams, tab: int,
    ...
    if tab == 0:
        ...
        d, st = find_peaks(amp, thr, r, min_dist_m=p.nms_min_dist_m)
        out["amp"] = amp
        out["thr"] = thr
        out["d"] = d
        out["st"] = st
...
def cfar_cells(p: SensorParams, n_points: int):
    """
    CFAR guard/pencere: FİZİKSEL MESAFE [m] -> hücre sayısı.
    Sabit nokta sayısı yerine metre kullanmanın sebebi: step_length değişince
    (ör. 1 -> 12) bir "hücre" 2.5 mm'den 30 mm'ye çıkar; sabit 16 nokta guard,
    4 cm iken birden 48 cm olur ve eşik saçmalar. Metre tabanlı olunca hangi
    step_length seçilirse seçilsin korunan/örneklenen fiziksel bölge aynı kalır.

      guard  : p.cfar_guard_m (0 -> darbe genişliğinden OTOMATİK: 2.5σ, profile göre)
      pencere: p.cfar_window_m (varsayılan 0.40 m)

    Dönüş: (eff_guard, eff_window) hücre cinsinden.
    """
    step_m = max(p.step_m(), 1e-6)
    # otomatik guard: darbe zarfının ~2.5σ'sı (kendini maskelemeyi önler)
    sigma_m = PROFILE_FWHM_M[p.profile] / 2.3548
    auto_guard_m = 2.5 * sigma_m
    guard_m = p.cfar_guard_m if p.cfar_guard_m > 0 else auto_guard_m
    guard_m = max(guard_m, auto_guard_m)      # otomatik taban: asla altına inme
    eff_guard = max(1, int(np.ceil(guard_m / step_m)))
    eff_window = max(2, int(np.ceil(max(p.cfar_window_m, 1e-6) / step_m)))
    # dizi sınırlarına sığdır: önce pencereyi (dizinin ~1/4'ünü aşmasın),
    # sonra guard'ı. Aksi halde pencere diziyi taşırıp guard'ı ezer.
    eff_window = max(2, min(eff_window, max(2, n_points // 4)))
    cap = max(1, n_points // 2 - eff_window - 2)
    eff_guard = min(eff_guard, cap)
    return eff_guard, eff_window
...
def compute_threshold(amplitude: np.ndarray, p: SensorParams,
    ...
    (noise_floor) alt sınır olarak kullanılır:
      ...
      "CFAR": yerel OS-CFAR; duyarlılık + FİZİKSEL guard/pencere ile uyarlanır.
          Guard ve referans penceresi NOKTA sayısıyla değil METRE ile verilir:
          step_length değişse bile aynı fiziksel bölge korunur (eşik "saçmalamaz").
          Yayılı clutter'da iyi; güçlü hedefin yanındaki zayıfı maskeleyebilir.
    ...
    if p.threshold_method == "CFAR":
        eff_guard, eff_window = cfar_cells(p, amplitude.size)
...
def find_peaks(amplitude: np.ndarray, threshold: np.ndarray,
               range_m: np.ndarray, min_dist_m: float = 0.0):
    """
    Eşiği aşan yerel maksimumları bul (alt-hücre doğruluk için parabolik
    interpolasyon), ardından NMS ile yan lobları temizle.

    NMS (Non-Maximum Suppression / Maksimum Olmayanları Bastırma):
      Bir darbenin omzunda ("ringing"/yan lob) beliren küçük tepecikler gerçek
      hedef değildir. Tepeleri güce göre büyükten küçüğe sıralarız; en güçlüden
      başlayıp, ondan min_dist_m'den yakın olan DAHA ZAYIF tepeleri bastırırız.
      Böylece her fiziksel hedef tek tespit üretir.
      min_dist_m <= 0 -> NMS kapalı (tüm tepeler döner).

    Dönüş: (mesafeler[m], güçler) — mesafeye göre yakından uzağa sıralı.
    """
    dists = []
    strengths = []
    n = amplitude.size
    # --- 1) tüm yerel maksimumlar (eşiğin üstünde) ---
    ...
    if min_dist_m <= 0 or len(dists) < 2:
        return np.array(dists), np.array(strengths)

    # --- 2) NMS: güçlüden zayıfa; yakın zayıf tepeleri (yan lob) bastır ---
    d = np.array(dists)
    s = np.array(strengths)
    order = np.argsort(s)[::-1]              # güce göre azalan
    d_s, s_s = d[order], s[order]
    suppressed = np.zeros(d_s.size, dtype=bool)
    keep_d, keep_s = [], []
    for i in range(d_s.size):
        if suppressed[i]:
            continue
        keep_d.append(float(d_s[i])); keep_s.append(float(s_s[i]))
        # kendinden zayıf ve min_dist_m'den yakın olanları ele
        near = (~suppressed) & (np.abs(d_s - d_s[i]) < min_dist_m)
        near[:i + 1] = False                 # kendini ve daha güçlüleri koru
        suppressed |= near

    fd = np.array(keep_d); fs = np.array(keep_s)
    by_dist = np.argsort(fd)                 # yakından uzağa sırala
    return fd[by_dist], fs[by_dist]
